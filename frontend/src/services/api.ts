...
export interface LoginSecuritySettings {
  ...
  ip_attempt_limit: number
  account_attempt_limit: number
...
export const api = {
  ...
  users: {
    ...
    create: (data: { email: string; name: string; password: string; role_id: number | null; organization_id?: number | null; org_ids?: number[] }) =>
  ...
  security: {
    getLoginSettings: () => request<LoginSecuritySettings>('/security/login'),
    updateLoginSettings: (data: Omit<LoginSecuritySettings, 'updated_by' | 'updated_at'>) =>
      request<LoginSecuritySettings>('/security/login', { method: 'PUT', body: JSON.stringify(data) }),
    listLoginAttempts: () => request<LoginAttemptSummary[]>('/security/login-attempts'),
    clearLoginAttempts: () => request<void>('/security/login-attempts', { method: 'DELETE' }),
  },
