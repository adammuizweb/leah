...
const modules = [
  ...
  { to: '/admin/security', icon: 'M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2h-1V7a5 5 0 00-10 0v4H6a2 2 0 00-2 2v6a2 2 0 002 2zm3-10v4H9V7a3 3 0 016 0z', label: 'Login Security', perm: '', rootOnly: true },
  ...
  { to: '/admin/bin', icon: 'M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16', label: 'Bin', perm: '', rootOnly: true },
]
...
export default function AdminLayout() {
  ...
  const visible = modules.filter(m => m.rootOnly ? user?.is_root : hasPermission(m.perm))
  ...
  return (
    <div className="flex gap-6 animate-fade-in">
      ...
      <div className="flex-1 min-w-0">
        <nav className="lg:hidden flex gap-2 overflow-x-auto pb-3 mb-4 border-b border-gray-200">
          {visible.map(m => (
            <NavLink
              key={m.to}
              to={m.to}
              className={({ isActive }) => `shrink-0 px-3 py-2 rounded-lg text-sm font-medium ${isActive ? 'bg-brand-50 text-brand-700' : 'bg-white text-gray-600 border border-gray-200'}`}
            >
              {m.label}
            </NavLink>
          ))}
