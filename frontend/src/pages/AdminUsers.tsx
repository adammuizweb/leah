...
export default function AdminUsers() {
  ...
  const create = useMutation({
    mutationFn: () => api.users.create({ email: form.email, name: form.name, password: form.password, role_id: form.role_id || null, org_ids: [...selectedOrgs] }),
  ...
  const unlock = useMutation({
    mutationFn: (id: number) => api.users.unlock(id),
    onSuccess: () => { toast('User unlocked', 'success'); queryClient.invalidateQueries({ queryKey: ['users'] }) },
    onError: (e: Error) => toast(e.message, 'error'),
  })
  ...
  function openEdit(u: User) {
    ...
    setSelectedOrgs(new Set(u.org_ids?.length ? u.org_ids : (u.organization_id ? [u.organization_id] : [])))
  }
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="divide-y divide-gray-200">
            {users?.map(u => {
              ...
              const canManage = !u.is_root || !!authUser?.is_root
              return (
                <tr key={u.id} className={u.deleted_at ? 'opacity-50' : ''}>
                  <td className="px-4 py-3 text-sm font-medium text-gray-900">{u.name}</td>
                  <td className="px-4 py-3 text-sm text-gray-600">{u.email}</td>
                  <td className="px-4 py-3 text-sm">{u.is_root ? <span className="text-purple-700 font-medium">Root</span> : u.role || '—'}</td>
