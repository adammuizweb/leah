...
export default function Tickets() {
  ...
  const canEdit = (t: Ticket) => user?.is_root || user?.role === 'admin' || t.created_by === user?.id
  const canBulk = user?.is_root || user?.role === 'admin' || permissions.includes('tickets.bulk_delete')
