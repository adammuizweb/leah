...
go -C backend build -o leah ./cmd/api
npm --prefix frontend run build

# Apply the production schema with a database-owner connection
export DATABASE_OWNER_URL='postgresql://OWNER_CONNECTION_TO_LEAH'
bash backend/migrations/migrate_schema.sh

# Create Root with private credentials
export ROOT_EMAIL='private-root@example.com'
read -r -s ROOT_PASSWORD && export ROOT_PASSWORD
go -C backend run ./cmd/bootstrap-root
unset ROOT_PASSWORD ROOT_EMAIL DATABASE_OWNER_URL

# Configure runtime values and start development
cp backend/.env.example backend/.env
# Replace DATABASE_URL and JWT_SECRET placeholders.
make dev-backend
\`\`\`

API tersedia di http://localhost:8080. Jalankan frontend dengan make dev-frontend dan buka http://localhost:5173. Dokumentasi lengkap tersedia di README repository.`,
  },
...
Root (is_root=true) bypass semua scope filter.
...
|------|-----------|-----------|
| Root | Semua holding/org | Semua holding/org |
...
1. **Root** — flag is_root=true di tabel users. Root bukan role dan bypass semua permission checks. Dibuat atau diperbarui oleh database owner melalui perintah bootstrap Root.
...
| Modul | Root | Superadmin | Admin | Agent | User |
|-------|-----------|------------|-------|-------|------|
...
Setiap modul memiliki permission CRUD terpisah. Bisa diatur lewat Admin → Permissions oleh Superadmin atau Root.`,
  },
  'asset-models-bulk': {
...
4. **Root untuk setup dan keamanan global saja** — sehari-hari pakai superadmin/admin dengan scope terbatas
...
Asset yang dihapus tidak langsung hilang — masuk ke **Bin** (soft delete). Root bisa merestore atau menghapus permanen dari Bin.
...
Root bypass semua scope filter dan bisa melihat semua asset di semua holding/organisasi.
...
Halaman Bin di **Admin → Bin** menampilkan daftar semua item yang dihapus dengan tombol Restore dan Delete Forever. Hanya Root yang bisa mengakses Bin.
