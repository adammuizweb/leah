...
function AdminRoute({ children }: { children: React.ReactNode }) {
  const { token, user } = useAuth()
  if (!token) return <Navigate to="/login" replace />
  const isAdmin = user?.role === 'admin' || user?.role === 'superadmin' || user?.is_root
...
function RootRoute({ children }: { children: React.ReactNode }) {
  const { token, user } = useAuth()
  if (!token) return <Navigate to="/login" replace />
  if (!user?.is_root) return <Navigate to="/admin" replace />
  return <>{children}</>
}
      ...
      <Routes>
        ...
        <Route element={<AppShell />}>
          ...
          <Route path="/admin" element={<AdminRoute><Admin /></AdminRoute>}>
            ...
            <Route path="bin" element={<RootRoute><AdminBin /></RootRoute>} />
            <Route path="security" element={<RootRoute><AdminSecurity /></RootRoute>} />
          </Route>
        </Route>
      </Routes>
      </ToastProvider>
