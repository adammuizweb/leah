...
psql "$LEAH_TEST_DATABASE_URL" -v ON_ERROR_STOP=1 -c 'DROP TABLE ticket_comments'

if ADOPT_EXISTING_SCHEMA=1 DATABASE_OWNER_URL="$LEAH_TEST_DATABASE_URL" \
    bash "$root_dir/backend/migrations/migrate_schema.sh"; then
    printf 'incomplete pre-ledger schema was incorrectly adopted\n' >&2
    exit 1
fi

test "$(psql "$LEAH_TEST_DATABASE_URL" -v ON_ERROR_STOP=1 -Atc 'SELECT COUNT(*) FROM schema_migrations')" = "0"
