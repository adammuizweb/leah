...
func (h *Handler) CreateTicket(w http.ResponseWriter, r *http.Request) {
	...
	if t.AssignedTo != nil && !requestCan(r, "tickets.assign") {
		respond(w, http.StatusForbidden, map[string]string{"error": "tickets.assign permission required"})
		return
	}
...
func (h *Handler) UpdateTicket(w http.ResponseWriter, r *http.Request) {
	...
	t.ID = id
	existing, err := h.svc.GetTicket(r.Context(), id)
	if err != nil {
		respond(w, http.StatusNotFound, map[string]string{"error": "not found"})
		return
	}
	if t.AssignedTo == nil {
		t.AssignedTo = existing.AssignedTo
	} else if !sameOptionalID(t.AssignedTo, existing.AssignedTo) && !requestCan(r, "tickets.assign") {
		respond(w, http.StatusForbidden, map[string]string{"error": "tickets.assign permission required"})
		return
	}
...
func (h *Handler) CreateAsset(w http.ResponseWriter, r *http.Request) {
	...
	if a.AssignedTo != nil && !requestCan(r, "assets.assign") {
		respond(w, http.StatusForbidden, map[string]string{"error": "assets.assign permission required"})
		return
	}
...
func (h *Handler) UpdateAsset(w http.ResponseWriter, r *http.Request) {
	...
	a.ID = id
	existing, err := h.svc.GetAsset(r.Context(), id)
	if err != nil {
		respond(w, http.StatusNotFound, map[string]string{"error": "not found"})
		return
	}
	if !sameOptionalID(a.AssignedTo, existing.AssignedTo) && !requestCan(r, "assets.assign") {
		respond(w, http.StatusForbidden, map[string]string{"error": "assets.assign permission required"})
		return
	}
...
func (h *Handler) ListTicketComments(w http.ResponseWriter, r *http.Request) {
	...
	if isRoot || role == "admin" || role == "superadmin" || contains(perms, "tickets.internal") {
...
func requestCan(r *http.Request, permission string) bool {
	if isRoot, _ := r.Context().Value(middleware.CtxKeyIsRoot).(bool); isRoot {
		return true
	}
	role, _ := r.Context().Value(middleware.CtxKeyUserRole).(string)
